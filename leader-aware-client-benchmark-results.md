...
- **LARGE_VALUES** (1 MiB values) read +22% on average, but a dedicated
  A/B/A rerun (official, leader-aware, leader-aware, official on a fresh
  cluster) breaks that reading into two parts. First, sporadic multi-second
  stalls — max latencies of exactly 5 s — hit both clients at random
  (one official record among them), and those stalls drove most of the
  apparent gap. Second, after excluding stall records, a small residual
  remains (~12.7 ms vs ~10.5 ms average), consistent with large payloads
  sharing one client-to-leader connection under HTTP/2 and TCP flow control
  where round-robin spreads the same bytes across three connections. The
  leader itself receives each value exactly once in both modes; this is a
  client-side transport effect. Treat it as a candidate cost for
  megabyte-scale writes, worth a dedicated experiment before opting such
  workloads in — not as a settled regression.
