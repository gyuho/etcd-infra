...
// RunConcurrencyMutexLockWithRevokedSession tests the ConcurrencyMutexLockWithRevokedSession scenario.
func RunConcurrencyMutexLockWithRevokedSession(runner Runner) {
	logutil.S().Infow("running", "scenario", ConcurrencyMutexLockWithRevokedSession.String())

	result := &Result{
		Scenario:  ConcurrencyMutexLockWithRevokedSession.String(),
	...
	case <-time.After(5 * time.Second):
		result.Success = false
		result.Output = "timed out waiting for m2 to acquire the lock"
